...
```{r import-data}
path_to_proteinGroups <- system.file("extdata/example_1_proteinGroups.txt", package = "proteoSE")

se <- proteoSE::se_read_in(file = path_to_proteinGroups, sep = "_rep_")
...
```{r import-data2}
path_to_design <- system.file("extdata/example_1_design.txt", package = "proteoSE")

design <- read.csv(file = path_to_design)

design

se <- proteoSE::se_read_in(file = path_to_proteinGroups, experimental_design = design)
...
## Imputation
Now that rows with too many missing values are excluded, we can replace NAs with values. The *DEP::impute()* function already provides a variety of options. The *proteoSE* package builds on that, but saves the raw data including NAs in another assay instead of overwriting it. In addition, a binary assay with 1 for imputed values and 0 for measured values is created to easily visualize missing data via iSEE afterwards. 
...
se_GO <- proteoSE::enrich_go_se(se_diff_spec, col_names = "griesser_heat_vs_no_glom_heat_diff", OrgDb = "org.Hs.eg.db", pvalueCutoff = 0.5)
