# iSEE custom panel classes (GeneAnnoTable, GOTable)
#
# Merged verbatim from isee_gat.R + isee_got.R during the 2026 refactor.
# iSEE provides the Panel/RowDataTable base classes and the panel-API generics
# (.defineInterface, .createObservers, ...) that these classes extend; methods
  ...
  output[[paste0(panel_name, "_annotation")]] <- renderUI({
    ...
    if (keytype!="ENTREZID") {
      ...
      if (requireNamespace(ORG, quietly=TRUE)) {
        orgdb <- getExportedValue(ORG, ORG)
        selgene_entrez <- try(AnnotationDbi::mapIds(orgdb, selectedGene, "ENTREZID", keytype),
                              silent=TRUE)
      }
    } else {
...
setMethod(".generateOutput", "GOTable", function(x, se, ..., all_memory, all_contents) {
  ...
  commands <- c("require(GO.db);",
                # Fully qualified on purpose: proteoSE imports dplyr::select, so a
