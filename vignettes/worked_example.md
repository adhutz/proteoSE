...
This walkthrough takes one made-up project from raw search-engine output all the
way to an interactive results browser, using nothing but `sev2` and the
Bioconductor packages it builds on. Every step shows the code you would run, the
console output it produces, and — where a picture helps — a real figure
generated from the bundled example data.
...
>
> The figures on this page are produced by
> `data-raw/make_worked_example_figures.R`; re-run it to regenerate them.
...
> **Heads-up — a name clash.** DEP2 *also* exports a `plot_volcano()`, so once
> `library(DEP2)` is attached it masks sev2's. When you want sev2's richer
> volcano, call it fully qualified: `sev2::plot_volcano(...)`.
...
**How many proteins did each run see?** `plot_numbers()` shows identified vs
quantified counts per sample — a first sanity check that no run underperformed.

![Proteins identified and quantified per sample](figures/01_numbers.png)

**How complete is the dataset?** `plot_frequency()` bins proteins by the number
of samples they were detected in. A big "seen in all 15" bar plus a tail of
partly-observed proteins is exactly what DIA data looks like.
...
First, look at *where* the missingness sits. `plot_missval()` draws the
missing-value pattern (black = measured, light = missing); `plot_detect()`
compares the intensity of proteins that are fully observed against those with
missing values.
...
That second plot is the key diagnostic: the proteins **with** missing values
(cyan) sit at systematically **lower** intensity than the complete ones. Missing
here is not random — it is low-abundance dropout (MNAR), which tells us how to
impute later.
...
narrowed Gaussian per sample — the classic Perseus approach, and the right
choice given the low-abundance dropout we just saw. Crucially, sev2 does **not**
overwrite your data: the raw matrix is preserved in its own assay, and a binary
mask records which cells were imputed.
...
`plot_imputation()` overlays the intensity distributions before (`se_filt`) and
after (`se_imp`) imputation. The imputed panel grows a lower-intensity shoulder —
the filled-in MNAR values landing where we expect them.

![Intensity distributions before and after imputation](figures/06_imputation.png)

`plot_normalization()` shows per-sample boxplots so you can confirm the samples
are comparable (and apply `normalize_vsn()` first if they are not).
...
With a complete matrix we can ask whether the samples group the way the design
says they should. `plot_pca()` projects the samples onto their main axes of
variation; `plot_cor()` shows the sample-to-sample correlation matrix.

![PCA of the 15 samples, coloured by condition](figures/08_pca.png)

The three conditions separate cleanly — Ctrl, TreatA and TreatB each form their
own tight cluster, and replicates sit together. That is the green light to test
...
sev2's `plot_volcano()` renders a publication-ready volcano straight from the
`rowData` results. `sig_from_column = TRUE` colours points using the
`_significant` column we just added, and the top hits are labelled automatically.

```r
# sev2:: because DEP2's plot_volcano is masking it (see Setup)
vA <- sev2::plot_volcano(se_diff, contrast = "TreatA_vs_Ctrl",
...
```r
sev2::plot_volcano(se_diff, contrast = "TreatB_vs_Ctrl",
                   sig_from_column = TRUE, top_n_sign = 12)$plot_out
```

![Volcano plot, TreatB vs Ctrl](figures/11_volcano_B.png)

A heatmap of just the significant proteins, k-means–clustered, shows the block
structure of the response — which proteins go up in which condition:
...
> sev2 also wraps this as `plot_heatmap_clustered()`, which additionally returns
> the per-cluster protein lists — handy when you want to feed one cluster into
> enrichment.
...
> *No figure shown here:* in this synthetic dataset the regulated proteins were
> chosen at random, so there is no real GO structure to recover. On a genuine
> experiment this call returns enriched stress-response and protein-folding
> categories among the up-regulated proteins.
...
Spectronaut report ─▶ optimized_spectronaut_to_se()  →  SE (4000 × 15)
                    ...
                    ─▶ sev2::plot_volcano() + heatmap →  visualise the response
