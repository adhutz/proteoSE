---
title: "The proteoSE Package"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{The proteoSE Package}
...
# proteoSE - Proteomics analysis on SummarizedExperiment

## Overview
The proteoSE package is a utility package that relies completely on the bioconductor packages DEP and iSEE/iSEEu for the analysis, adding only convenience functions, imputation methods and additional iSEE containers.
...
#All other dependencies are installed automatically with proteoSE. 
...
se <- proteoSE::se_read_in(system.file("extdata", "example_1_proteinGroups.txt", package = "proteoSE"), sep = "_rep_")
...
```{r import-data2}
design <- read.csv(file = system.file("extdata", "example_1_design.txt", package = "proteoSE"))

design

se <- proteoSE::se_read_in(system.file("extdata", "example_1_proteinGroups.txt", package = "proteoSE"), experimental_design = design)
...
se_filtered <- proteoSE::filter_perseus(se, perc_na = 0.33, filter_mode = "each_group")
```

## Imputation
Now that rows with too many missing values are excluded, we can replace NAs with values. The *dep2::impute()* function already provides a variety of options. The *proteoSE* package builds on that, but saves the raw data including NAs in another assay instead of overwriting it. In addition, a binary assay with 1 for imputed values and 0 for measured values is created to easily visualize missing data via iSEE afterwards. 

### Possibility 1: Use DEP standard functions
```{r impute DEP}
se_imp <- proteoSE::impute_DEP2(se_filtered, fun="QRILC")
```

### Possibility 2: Use mixed imputation for **MAR** vs. **MNAR**
It is possible to distinguish different types of "missingness", termed either **missing at random (MAR)** or **missing not at random (MNAR)**. The *DEP* package already allows to select different imputation methods depending on if rows contain **MAR** or **MNAR** values via the *mixed* argument. For this, the column *randna* is necessary in the rowData that specifies the type of missingness. This column can now be added via the **proteoSE::add_randna()** function.

```{r}
se_randna <- proteoSE::add_randna(se_filtered)

se_imp <- proteoSE::impute_DEP2(se_randna, fun = "mixed", mar = "knn", mnar = "QRILC")
```
...
```{r impute-perseus}
se_imp <- proteoSE::impute_perseus(se_filtered)
...
se_diff_ctrl <- test_diff(se_imp, type = "control", control = "cond_a")
se_diff_ctrl <- add_rejections(se_diff_ctrl, alpha = 0.05, lfc = 1)

# Possibility 3: Use DEP method to test specific contrast 
se_diff_spec <- test_diff(se_imp, type = "manual", test = "cond_b_vs_cond_a")
se_diff_spec <- add_rejections(se_diff_spec, alpha = 0.05, lfc = 1)
```

## GO-term Enrichment
The *proteoSE::enrich_go_se()* wrapper around *clusterProfiler::gseGO()* is used to perform GO-term enrichment analysis. In theory, any numerical column can be selected as *col_name*, although only lfq_data or even better fold changes should be used. If no col_name is provided, all columns ending with "_diff" are selected. Results are stored in the *metadata* of the provided SummarizedExperiment. 

```{r GO-term-enrichment}

se_GO <- proteoSE::enrich_go_se(se_diff_spec,
                                col_names = "cond_b_vs_cond_a_diff",
                                OrgDb = "org.Hs.eg.db",
                                pvalueCutoff = 0.5)
...
isee_object <- proteoSE::se_to_isee(se_GO)
...
```{r iSEE-app}
# create app with custom plots
app <- proteoSE::isee_mini(isee_object)
...
```{r reduce-rowData}
minimal_data <- proteoSE::make_mini(isee_object)
```

## Recording the analysis

Everything above is a chain of function calls, and the object at the end
remembers the *data* but not the calls. A pipeline records the calls instead of
running them, so the analysis can be inspected, saved alongside the results,
replayed, and turned into a first draft of a methods paragraph.

```{r pipeline-build}
pipe <- proteoSE::proteoSE_pipeline(se) |>
  proteoSE::add_step("filter_perseus", perc_na = 0.33, filter_mode = "each_group") |>
  proteoSE::add_step("impute_perseus") |>
  proteoSE::add_step("test_diff", type = "manual", test = "cond_b_vs_cond_a") |>
  proteoSE::add_step("add_rejections", alpha = 0.05, lfc = 1)

pipe
```

Nothing has run yet. `run_pipeline()` folds the steps over the input object,
and reproduces the `se_diff_spec` built by hand above:

```{r pipeline-run}
pipe <- proteoSE::run_pipeline(pipe)

replayed <- proteoSE::pipeline_result(pipe)
all.equal(
  SummarizedExperiment::rowData(replayed)$cond_b_vs_cond_a_diff,
  SummarizedExperiment::rowData(se_diff_spec)$cond_b_vs_cond_a_diff
)
```

`write_methods()` turns the same recipe into prose. Read it before it goes into
a manuscript -- it is a template fill, not a co-author.

```{r pipeline-methods}
proteoSE::write_methods(pipe)
```
...
```{r save-results}
f <- file.path(tempdir(), "isee_object.rda")
save(isee_object, file = f)
