...
se <- sev2::se_read_in("../inst/extdata/example_1_proteinGroups.txt", sep = "_rep_")
...
se <- sev2::se_read_in("../inst/extdata/example_1_proteinGroups.txt", experimental_design = design)
...
se_filtered <- sev2::filter_perseus(se, perc_na = 0.33, filter_mode = "each_group")
```
...
```{r impute-perseus}
se_imp <- sev2::impute_perseus(se_filtered)
...
se_GO <- sev2::se_GOE(se_diff_spec, col_names = "griesser_heat_vs_no_glom_heat_diff", OrgDb = "org.Hs.eg.db", pvalueCutoff = 0.5)
...
isee_object <- sev2::se_to_isee(se_GO)
...
```{r iSEE-app}
# create app with custom plots
app <- sev2::isee_mini(isee_object)
...
```{r reduce-rowData}
minimal_data <- sev2::make_mini(isee_object)
```
